...
class TradingAgentsGraph:
    ...
    def _create_tool_nodes(self) -> Dict[str, ToolNode]:
        """Create tool nodes for different data sources using A股 tools."""
        return {
            "market": ToolNode(
                [
                    get_stock_data,
    ...
    def _fetch_returns(
        ...
        try:
            ...
            stock_df = get_daily_kline(ticker, trade_date, end_str)
            bench_df = get_index_kline(benchmark, trade_date, end_str)

            if stock_df.empty or bench_df.empty or len(stock_df) < 2 or len(bench_df) < 2:
