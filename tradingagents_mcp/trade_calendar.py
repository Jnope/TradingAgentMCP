    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if len(date_str) == 10 and "-" in date_str:
                ...
                continue
            if start.replace("-", "") <= date_str.replace("-", "") <= end.replace("-", ""):
        ...
        logger.warning(f"akshare查询交易日历失败: {e}")
        return None
...
def _llm_judge_trade_date(date_str: str) -> str:
    try:
        ...
        if not api_key:
            raise ValueError("LLM API Key 未设置")
...
def get_trade_dates() -> Set[str]:
    ...
    if akshare_data:
        ...
        _module_cache = set(akshare_data["trade_dates"])
        return _module_cache
